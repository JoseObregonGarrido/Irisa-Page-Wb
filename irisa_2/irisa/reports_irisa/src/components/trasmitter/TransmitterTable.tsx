...
const TransmitterTable: React.FC<TransmitterTableProps> = ({ 
    ...
    const getLayoutConfig = () => {
        switch (outputUnit) {
            case 'mv':
                return {
                    gridCols: 'lg:grid-cols-[1fr_1fr_1fr_140px_1fr_60px]', 
                    minWidth: 'lg:min-w-[800px]',
            ...
            case 'ohm':
                const ohmCols = hasUeTransmitter ? 10 : 8; // Sincronizado con el renderizado
                return {
                    gridCols: hasUeTransmitter ? 'lg:grid-cols-[1fr_1fr_1fr_1fr_1fr_1fr_1fr_1fr_1fr_60px]' : 'lg:grid-cols-[1fr_1fr_1fr_1fr_1fr_1fr_1fr_60px]',
                    minWidth: 'lg:min-w-[1100px]',
                    headers: ['Ideal UE', 'Ideal mA', 'Ideal Ohm', 'Patrón UE', ...(hasUeTransmitter ? ['UE Trans.'] : []), 'mA Sensor', 'Ohm Sensor', ...(hasUeTransmitter ? ['Err UE'] : []), 'Err Ohm', 'Acción']
                };
            default: // mA
                return {
                    gridCols: hasUeTransmitter ? 'lg:grid-cols-[1fr_1fr_1fr_1fr_1fr_1fr_1fr_1fr_60px]' : 'lg:grid-cols-[1fr_1fr_1fr_1fr_1fr_1fr_60px]',
                    minWidth: 'lg:min-w-[950px]',
                    headers: ['Ideal UE', 'Ideal mA', 'Patrón UE', ...(hasUeTransmitter ? ['UE Trans.'] : []), 'mA Trans.', ...(hasUeTransmitter ? ['Err UE'] : []), 'Err %', 'Acción']
                };
        }
    };
                ...
                <div className={`w-full ${config.minWidth}`}>
                    ...
                    <div className={`hidden lg:grid ${config.gridCols} bg-gray-50 border-b border-gray-200 text-[10px] font-bold text-gray-500 tracking-wider`}>
                        {config.headers.map((h, i) => (
                            <div key={i} className={`px-2 py-4 text-center flex items-center justify-center ${h.includes('Err') ? 'bg-red-50 text-red-700' : ''}`}>{h}</div>
                        ))}
                    </div>
                            ...
                            <div key={index} className={`bg-white p-4 lg:p-0 lg:grid ${config.gridCols} lg:items-center hover:bg-teal-50/30 transition-colors`}>
                                ...
                                {outputUnit === 'mv' ? (
                                    <>
                                        ...
                                        <div className="lg:px-2 lg:py-3"><InputField label="mV sensor" unit="mV" value={m.sensormV} onChange={(e:any) => handleChange(index, 'sensormV', e.target.value)} /></div>
                                        <div className="lg:px-2 lg:py-3 flex justify-center">
                                            <div className="flex gap-2 bg-gray-100 p-1 rounded-lg border border-gray-200 shadow-sm">
                                                {['J', 'K'].map((type) => (
                                                    <label key={type} className="flex items-center gap-1 cursor-pointer px-1">
                                                        <input type="radio" checked={m.sensorType === type} onChange={() => handleChange(index, 'sensorType', type)} className="w-3 h-3 accent-orange-600" />
                                                        <span className={`text-[10px] font-bold ${m.sensorType === type ? 'text-orange-700' : 'text-gray-400'}`}>{type}</span>
                                                    </label>
                                                ))}
                                            </div>
                                        </div>
                                        <div className="lg:px-2 lg:py-3 lg:bg-red-50/20"><InputField label="Error mV" unit="mV" value={m.errormV} isError readOnly /></div>
                                    </>
                                ) : outputUnit === 'tx' ? (
                                    <>
                                        <div className="lg:px-2 lg:py-3"><InputField label="Ideal UE" unit="UE" value={m.idealUE} onChange={(e:any) => handleChange(index, 'idealUE', e.target.value)} /></div>
                                ...
                                <div className="flex justify-center items-center py-2 lg:py-0 border-t lg:border-t-0 mt-2 lg:mt-0">
                                    <button onClick={() => onMeasurementsChange(measurements.filter((_, i) => i !== index))} className="text-red-500 hover:bg-red-50 p-2 rounded-full transition-colors" title="Eliminar fila">
