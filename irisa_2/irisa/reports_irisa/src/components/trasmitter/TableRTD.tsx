...
export const TableRTD = ({ measurements, onMeasurementsChange, hasUeTransmitter }: any) => {
    // SIN UE (10 columnas): Ideal UE, Ideal mA, Patron UE, UE Trans, mA Trans, % Rango, Err UE, Err mA, Err %, Accion
    // CON UE (11 columnas): Ideal UE, Ideal mA, Ideal Ohm, Patron UE, mA Sensor, Ohm Sensor, % Rango, Err mA, Err %, Err Ohm, Accion
    ...
    const handleChange = (index: number, field: string, value: string) => {
        ...
        // Parseo de valores para calculos
        ...
        // --- Calculos de Error (Valor Medido - Valor Ideal) ---
        
        // 1. Error en Unidades de Ingenieria (Temperatura)
        updatedRow.errorUE = (ueTrans - idealUE).toFixed(3);
        
        // 2. Error en Resistencia (Ohmios)
        updatedRow.errorOhm = (ohmTrans - idealOhm).toFixed(3);
        
        // 3. Error en Corriente (mA)
        const diffMA = maTrans - idealmA;
        updatedRow.errormA = diffMA.toFixed(3);
        
        // 4. Error Porcentual (Respecto al Span de 16mA del lazo 4-20mA)
        // Formula: ((mA Real - mA Ideal) / 16) * 100
        updatedRow.errorPercentage = ((diffMA / 16) * 100).toFixed(2);
        ...
        <div className="overflow-x-auto">
            <div className={`min-w-full ${hasUeTransmitter ? 'lg:min-w-[1200px]' : 'lg:min-w-[1000px]'}`}>
                ...
                <div className={`hidden lg:grid ${gridCols} bg-gray-50 border-b border-gray-200 text-[10px] font-bold text-gray-500 uppercase tracking-wider`}>
                    ...
                    <div className="px-2 py-4 text-center">Patron UE</div>
                    ...
                    <div className="px-2 py-4 text-center">Accion</div>
                </div>
                ...
                <div className="p-3 lg:p-0 space-y-4 lg:space-y-0 lg:divide-y lg:divide-gray-200 bg-gray-100 lg:bg-white">
                    {measurements.map((m: any, i: number) => (
                        <div key={i} className="bg-white rounded-xl lg:rounded-none shadow-sm lg:shadow-none border border-gray-200 lg:border-none overflow-hidden">
                            {/* Version Movil: Cabecera de fila */}
                            <div className="flex lg:hidden justify-between items-center px-4 py-2 bg-gray-50 border-b border-gray-100">
                                <span className="text-[10px] font-black text-teal-700 uppercase">Medicion RTD {i + 1}</span>
                            ...
                            <div className="overflow-x-auto">
                                <div className={`flex flex-nowrap lg:grid ${gridCols} lg:items-center p-4 lg:p-0 gap-4 lg:gap-0 min-w-max lg:min-w-full`}>
                                    ...
                                    <div className="lg:px-2 lg:py-3"><InputField label="Patron UE" unit="UE" value={m.patronUE} onChange={(e:any) => handleChange(i, 'patronUE', e.target.value)} /></div>
                                    ...
                                    <div className="hidden lg:flex justify-center">
                                        <button onClick={() => onMeasurementsChange(measurements.filter((_:any, idx:any) => idx !== i))} className="text-red-500 hover:bg-red-50 p-2 rounded-full transition-colors">
