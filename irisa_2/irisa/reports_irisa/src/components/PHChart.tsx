...
const PHChart = forwardRef(({ tests }: PHChartProps, ref) => {
    ...
    const vidaData = tests.map((t, i) => ({
        name:       `pH ${t.patron || '?'} M${i + 1}`,
        rangoVida:  parseFloat(t.errorMv) || 0,  // T = 30-(V-174)
        patron:     t.patron || '?',
        promedio:   parseFloat(t.promedio) || 0,
        ...
        if (active && payload?.length) {
            ...
            return (
                <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-3 text-xs">
                    ...
                    <p style={{ color: PATRON_COLORS[patronVal] || PURPLE }}>Voltaje: <span className="font-bold">{d?.voltaje} mV</span></p>
        ...
        if (active && payload?.length) {
            ...
            return (
                <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-3 text-xs">
                    ...
                    <p className="text-orange-500">Rango Vida T: <span className="font-bold">{rv.toFixed(2)} mV</span></p>
                    <p className="text-gray-400 text-[10px]">T = 30 − ({d?.voltaje} − {V0}) = {rv.toFixed(2)}</p>
                                ...
                                <div key={patron} className="border border-gray-100 rounded-xl p-3 bg-gray-50/50">
                                    ...
                                    <ResponsiveContainer width="100%" height={220}>
                                        <LineChart data={data} margin={{ top: 10, right: 20, left: 5, bottom: 10 }}>
                                            ...
                                            <Line type="monotone" dataKey="voltaje" stroke={color} strokeWidth={2}
                                                dot={{ fill: color, r: 5, strokeWidth: 2, stroke: '#fff' }} activeDot={{ r: 7 }} isAnimationActive={false} />
                                        </LineChart>
                                    </ResponsiveContainer>
                                    {data.length > 0 && (() => {
            ...
            <div ref={chart3Ref} className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
                ...
                <ResponsiveContainer width="100%" height={280}>
                    <BarChart data={vidaData} margin={{ top: 10, right: 30, left: 10, bottom: 20 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                        <XAxis dataKey="name" tick={{ fontSize: 9 }} interval={0} angle={-10} />
                        <YAxis label={{ value: 'Rango Vida (mV)', angle: -90, position: 'insideLeft', offset: 10, fontSize: 11 }}
