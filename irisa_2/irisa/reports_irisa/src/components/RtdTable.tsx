...
const TableInput = ({ label, value, onChange, unit, isError = false, readOnly = false }: any) => (
                ...
                <table className="w-full text-left border-collapse min-w-max">
                    <thead className="bg-gray-50 border-b border-gray-200">
                        <tr className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">
                            ...
                            <th className="px-2 py-4 text-center">Acción</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200">
                        {measurements.map((m: any, index: number) => (
                            <tr key={index} className="hover:bg-gray-50">
                                ...
                                {hasUeTransmitter && <td className="p-2"><TableInput unit="UE" value={m.ueTransmitter} onChange={(e:any) => onChange(index, 'ueTransmitter', e.target.value)} /></td>}
                ...
                {measurements.map((m: any, index: number) => (
                    <div key={index} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 relative">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            {hasUeTransmitter && <TableInput label="UE Tx" unit="UE" value={m.ueTransmitter} onChange={(e:any) => onChange(index, 'ueTransmitter', e.target.value)} />}
                            ...
                            <TableInput label="Err mA" unit="mA" value={m.errormA} readOnly isError />
                            <TableInput label="Err %" unit="%" value={m.errorPercentage} readOnly isError />
                        </div>
                    </div>
                ))}
            </div>
