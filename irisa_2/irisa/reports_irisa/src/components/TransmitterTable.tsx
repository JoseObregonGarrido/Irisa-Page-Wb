                        ...
                        {measurements.map((measurement, index) => (
                            <tr key={index} className="hover:bg-gray-50 transition-colors duration-150">
                                {Object.keys(measurement).map((key, keyIndex) => {
                                    ...
                                    return (
                                        <td key={key} className={`px-3 py-3 whitespace-nowrap ${isErrorColumn ? 'bg-red-50' : ''}`}>
                                            <div className="relative">
                                                <input
                                                    type="text"
                                                    value={measurement[key as keyof Measurement]}
                                                    onChange={(e) => handleChange(index, key as keyof Measurement, e.target.value)}
                                                    className={`w-full px-2 py-2 pr-8 text-sm border rounded-lg focus:outline-none focus:ring-2 transition-colors ${
                                                        isErrorColumn 
                                                            ? 'border-red-300 focus:ring-red-500 focus:border-red-500 bg-red-50' 
                                                            : 'border-gray-300 focus:ring-teal-500 focus:border-teal-500 bg-white'
                                                    }`}
                                                    placeholder="0.00"
                                                    readOnly={isErrorColumn} 
                                                />
                                                {fieldLabels[key] && (
                                                    <span className={`absolute right-2 top-2 text-xs ${
                                                        isErrorColumn ? 'text-red-500' : 'text-gray-500'
                                                    }`}>
                                                        {fieldLabels[key]}
                                                    </span>
                                                )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="flex items-center text-sm">
                            ...
                            <span className="text-blue-600 font-medium">
                                {measurements.length > 0 ? `${Math.min(...measurements.map(m => parseFloat(m.maTransmitter) || 0)).toFixed(2)} - ${Math.max(...measurements.map(m => parseFloat(m.idealUe) || 0)).toFixed(2)} UE` : 'N/A'}
                            </span>
                        </div>
                        <div className="flex items-center text-sm">
                            <span className="text-gray-600 mr-2">Rango mA:</span>
                            <span className="text-red-600 font-medium">
                                {measurements.length > 0 ? `${Math.min(...measurements.map(m => parseFloat(m.maTransmitter) || 0)).toFixed(2)} - ${Math.max(...measurements.map(m => parseFloat(m.maTransmitter) || 0)).toFixed(2)} mA` : 'N/A'}
                            </span>
                        </div>
                    </div>
                </div>
