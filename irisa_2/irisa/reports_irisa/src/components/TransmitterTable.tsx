...
const InputField = ({ label, value, onChange, unit, isError = false, readOnly = false }: any) => (
    <div className="flex flex-col w-full min-w-[80px]"> {/* Agregado min-w para que no se colapse */}
        ...
        <div className="relative w-full">
            <input
                ...
                onChange={onChange}
                readOnly={readOnly}
                className={`w-full px-2 py-2 pr-7 text-sm border rounded-lg focus:outline-none focus:ring-2 
                    ${isError 
...
const TransmitterTable: React.FC<TransmitterTableProps> = ({ 
    ...
    return (
        <div className="mt-8 w-full border border-gray-200 rounded-xl shadow-lg bg-white overflow-hidden">
            {/* HEADER DE LA TABLA */}
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="flex flex-wrap items-center gap-4 text-white">
                        <div className="flex items-center">
                            ...
                            <h3 className="text-lg font-bold tracking-tight">Mediciones</h3>
                        </div>
            ...
            {/* CONTENEDOR CON SCROLL HORIZONTAL PARA QUE NO SE DEFORME */}
            <div className="overflow-x-auto w-full">
                <div className="min-w-[1200px]"> {/* Forzamos un ancho mínimo para que las columnas respiren */}
                    ...
                    <div className={`hidden lg:grid ${gridLayout} bg-gray-50 border-b border-gray-200 text-[10px] font-bold text-gray-600 uppercase tracking-wider`}>
                        ...
                        {hasUeTransmitter && <div className="px-2 py-4 text-center bg-red-50 text-red-600 border-l border-red-100">Err UE</div>}
                        <div className={`px-2 py-4 text-center bg-red-50 text-red-600 ${!hasUeTransmitter ? 'border-l border-red-100' : ''}`}>Err {outputUnit}</div>
                        <div className="px-2 py-4 text-center bg-red-50 text-red-600">Err %</div>
                    ...
                    <div className="divide-y divide-gray-100">
                        {measurements.map((m, index) => (
                            <div key={index} className={`grid grid-cols-1 lg:grid ${gridLayout} items-center hover:bg-gray-50/80 transition-colors p-4 lg:p-0`}>
                                <div className="grid grid-cols-2 md:grid-cols-3 lg:contents gap-3">
                                    ...
                                    {/* SECCIÓN DE ERRORES */}
                                    {hasUeTransmitter && (
                                        <div className="lg:px-2 lg:py-3 lg:bg-red-50/50 lg:border-l lg:border-red-100"><InputField label="Err UE" unit="UE" value={m.errorUe} isError readOnly /></div>
                                    )}

                                    <div className={`lg:px-2 lg:py-3 lg:bg-red-50/50 ${!hasUeTransmitter ? 'lg:border-l lg:border-red-100' : ''}`}><InputField label={`Err ${outputUnit}`} unit={outputUnit} value={m.errorMa} isError readOnly /></div>
                                    <div className="lg:px-2 lg:py-3 lg:bg-red-50/50"><InputField label="Err %" unit="%" value={m.errorPercentage} isError readOnly /></div>

                                    {/* BOTÓN ELIMINAR */}
                                    <div className="flex justify-center items-center py-2 lg:py-0">
                                        <button type="button" onClick={() => handleDeleteRow(index)} className="p-2 text-gray-400 hover:text-red-600 transition-colors rounded-full hover:bg-red-50">
