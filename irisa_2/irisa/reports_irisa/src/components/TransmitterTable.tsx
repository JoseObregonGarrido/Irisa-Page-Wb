...
// OPTIMIZACIÓN MOBILE: Label lateral y reducción de altura
const InputField = ({ label, value, onChange, unit, isError = false, readOnly = false }: any) => (
    <div className="flex flex-row lg:flex-col items-center lg:items-start w-full gap-2 lg:gap-0">
        <label className="block text-[9px] font-black text-gray-400 uppercase lg:hidden w-24 shrink-0 leading-tight">
            {label}
        </label>
            ...
            <div className="bg-gradient-to-r from-teal-600 to-emerald-600 px-4 py-4 sm:px-6 rounded-t-xl lg:rounded-none">
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div className="flex flex-wrap items-center gap-3">
                        <h3 className="text-lg font-bold text-white tracking-tight leading-tight">Mediciones de transmisor</h3>
                        <div className="flex bg-black/20 p-1 rounded-lg border border-white/10 shrink-0">
                            <button type="button" onClick={() => setOutputUnit('mA')} className={`px-3 py-1 text-[10px] font-bold rounded-md transition-all ${outputUnit === 'mA' ? 'bg-white text-teal-700 shadow' : 'text-white hover:bg-white/10'}`}>mA</button>
                            <button type="button" onClick={() => setOutputUnit('ohm')} className={`px-3 py-1 text-[10px] font-bold rounded-md transition-all ${outputUnit === 'ohm' ? 'bg-white text-teal-700 shadow' : 'text-white hover:bg-white/10'}`}>RTD</button>
                        </div>
                        <button
                            type="button"
                            onClick={() => setHasUeTransmitter(!hasUeTransmitter)}
                            className={`px-3 py-1.5 rounded-lg border text-[10px] font-bold transition-all shrink-0 ${hasUeTransmitter ? 'bg-white text-teal-700 shadow' : 'bg-white/10 border-white/20 text-white hover:bg-white/20'}`}
                        >
                            {hasUeTransmitter ? 'Ocultar UE' : 'Mostrar UE'}
                        </button>
                    </div>
                    <button onClick={() => onMeasurementsChange([...measurements, { percentage: "", idealUE: "", patronUE: "", ueTransmitter: "", idealmA:"", idealohm: "", maTransmitter: "", ohmTransmitter: "", errorUE: "", errormA: "", errorPercentage: "", errorOhm: "" }])} className="w-full md:w-auto px-4 py-2 bg-white text-teal-700 font-bold rounded-lg shadow-md hover:bg-teal-50 text-sm active:scale-95 transition-transform">Nueva fila</button>
                </div>
            </div>
                    ...
                    <div className="p-3 lg:p-0 space-y-4 lg:space-y-0 lg:divide-y lg:divide-gray-200 bg-gray-50 lg:bg-white">
                        {measurements.map((m, index) => (
                            <div key={index} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 lg:border-none lg:rounded-none lg:p-0 transition-all">
                                {/* Badge de Medición y Botón Borrar en Mobile */}
                                ...
                                <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid ${gridCols} lg:items-center gap-x-4 gap-y-2 lg:gap-0`}>
                                    ...
                                    {/* SECCIÓN DE ERRORES: Compacta en Mobile */}
                                    <div className="grid grid-cols-1 gap-2 mt-2 pt-2 border-t border-dashed border-gray-100 lg:contents">
                                        ...
                                        {isOhm && <div className="lg:px-2 lg:py-3 lg:bg-red-50/20"><InputField label="Error Ω" unit="Ω" value={m.errorOhm} isError readOnly /></div>}
