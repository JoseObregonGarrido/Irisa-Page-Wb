...
// --- CONFIGURACIÓN DE COLORES POR EQUIPO ---
const THEMES = {
    TRANSMITTER: { primary: [20, 110, 90] as [number, number, number], title: 'TRANSMISOR' }, // Verde
    THERMOSTAT: { primary: [234, 88, 12] as [number, number, number], title: 'TERMOSTATO' }, // Naranja
    PRESSURE: { primary: [30, 64, 175] as [number, number, number], title: 'PRESOSTATO' }    // Azul
};
...
export const generatePressureSwitchPDF = async (data: any, tests: any[], chartImages?: string[]) => {
    ...
    autoTable(pdf, {
        startY: (pdf as any).lastAutoTable.finalY + 10,
        head: [[`P. Disparo (${data.unity || 'PSI'})`, `P. Repone (${data.unity || 'PSI'})`, 'Diferencial', 'Contacto']],
        body: tests.map((t) => {
            const diff = Math.abs(parseFloat(t.presionDisparada) - parseFloat(t.presionRepone)).toFixed(2);
            return [
                t.presionDisparada,
                t.presionRepone,
                isNaN(parseFloat(diff)) ? '0.00' : diff,
                `${t.isNO ? 'N.O' : ''} ${t.isNC ? 'N.C' : ''}`.trim() || 'N/A'
            ];
        }),
